...
class DrawInformation:  # global variables are define here
    BLACK = 0, 0, 0
    WHITE = 255, 255, 255
    GREEN = 0, 255, 0
    RED = 255, 0, 0
    ...
    def __init__(self, width, height, lst):  # creat a window
        self.width = width
        self.height = height
...
def draw(draw_info, algo_name, ascending):
    ...
    control = draw_info.FONT.render("R - Reset | SPACE - Start Sorting | A - Ascending | D - Descending", 1,
                                    draw_info.BLACK)
...
def main():
    run = True  # to creat infinite loop
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type != pygame.KEYDOWN:
                continue
            if event.key == pygame.K_r:  # reset the block of sorting list
                lst = generate_starting_list(n, min_val, max_val)
